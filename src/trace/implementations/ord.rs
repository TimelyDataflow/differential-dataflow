...
use timely_sort::{Unsigned, LSBRadixSorter, RadixSorter, RadixSorterBase};
...
type OrdBuilder<Key, Val, Time, R> = OrderedBuilder<Key, OrderedBuilder<Val, UnorderedBuilder<(Time, R)>>>;
...
#[derive(Debug)]
pub struct Spine<Key: Ord, Val: Ord, Time: Lattice+Ord, R: Ring> {
...
#[derive(Debug)]
pub struct Layer<Key: Ord, Val: Ord, Time: Lattice+Ord, R: Ring> {
	/// Where all the dataz is.
	pub layer: OrderedLayer<Key, OrderedLayer<Val, UnorderedLayer<(Time, R)>>>,
	/// Description of the update times this layer represents.
	pub desc: Description<Time>,
}

impl<Key: Clone+Ord+Hashable, Val: Ord+Clone, Time: Lattice+Ord+Clone, R: Ring> Batch<Key, Val, Time, R> for Rc<Layer<Key, Val, Time, R>> {
...
impl<Key: Clone+Ord+Hashable, Val: Ord+Clone, Time: Lattice+Ord+Clone, R: Ring> Layer<Key, Val, Time, R> {
	...
	pub fn advance_by(layer: &Rc<Self>, frontier: &[Time]) -> Self { 
		...
		let mut builder = <OrdBuilder<Key, Val, Time, R> as TupleBuilder>::with_capacity(layer.len());
...
#[derive(Debug)]
pub struct LayerCursor<Key: Clone+Ord, Val: Ord+Clone, Time: Lattice+Ord+Clone, R: Copy> {
	cursor: OrderedCursor<Key, OrderedCursor<Val, UnorderedCursor<(Time, R)>>>,
}


impl<Key: Clone+Ord, Val: Ord+Clone, Time: Lattice+Ord+Clone, R: Copy> Cursor<Key, Val, Time, R> for LayerCursor<Key, Val, Time, R> {
...
where Key: Clone+Ord+Hashable, Val: Ord+Clone, Time: Lattice+Ord+Clone, R: Ring {
	fn new() -> Self { 
		LayerBatcher { 
			buffers: Vec::new(),
	...
	fn push_batch(&mut self, batch: &mut Vec<((Key, Val), Time, R)>) {
	...
	fn seal(&mut self, upper: &[Time]) -> Rc<Layer<Key, Val, Time, R>> {
		...
		if to_seal.len() > 1 {
			...
			self.sorter.recycle(&mut to_seal);
			consolidate(&mut self.stage, 0);
			for ((key, val, time), diff) in self.stage.drain(..) {
				builder.push((key, val, time, diff));
			}
...
/// A builder for creating layers from unsorted update tuples.
pub struct LayerBuilder<Key: Ord, Val: Ord, Time: Ord, R> {
	builder: OrdBuilder<Key, Val, Time, R>,
}

impl<Key, Val, Time, R> Builder<Key, Val, Time, R, Rc<Layer<Key, Val, Time, R>>> for LayerBuilder<Key, Val, Time, R> 
where Key: Clone+Ord+Hashable, Val: Ord+Clone, Time: Lattice+Ord+Clone, R: Ring {

	fn new() -> Self { LayerBuilder { builder: OrdBuilder::new() } }
