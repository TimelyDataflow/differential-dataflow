...
pub trait Batch : BatchReader where Self: ::std::marker::Sized {
    ...
    /// Merges this batch with a consecutive empty batch.
    ///
    /// This method places the following to be true:
    ///  * `other` is empty.
    ///  * Either `self.lower() == other.upper()` or `self.upper() == other.lower()`.
    ///
    /// Implementers should panic if any of these requirements is violated.
    fn merge_empty(self, other: &Self) -> Self;
}
