...
// pub struct LayerKeyCursor<Key: Ord+Debug, Val: Ord+Debug, Time: Lattice+Ord+Debug> {
// 	key: usize,
// 	layer: Rc<Layer<Key, Val, Time>>,	
// }

// impl<'a, Key: Ord+Debug+'a, Val: Ord+Debug+'a, Time: Lattice+Ord+Debug+'a> KeyCursor<'a, Key, Val, Time> for LayerKeyCursor<Key, Val, Time> {
// 	type ValCursor = LayerValCursor<'a, Key, Val, Time>;
// 	fn next_key(&'a mut self) -> Option<Self::ValCursor> {
// 		if self.key < self.layer.keys.len() {
// 			let val_lower = if self.key == 0 { 0 } else { self.layer.offs[self.key-1] };
// 			let val_upper = self.layer.offs[self.key];
// 			let time_lower = if val_lower == 0 { 0 } else { self.layer.vals[val_lower-1].1 };

// 			let result = LayerValCursor {
// 				key: &self.layer.keys[self.key],
// 				off: time_lower,
// 				vals: &self.layer.vals[val_lower .. val_upper],
// 				times: &self.layer.times[..],
// 			};
...
// 		self.key += advance(&self.layer.keys[self.key ..], |k| k.lt(key));
// 		if self.peek_key().map(|k| k == key) == Some(true) {
// 			self.next_key()
// 		}
// 		else {
// 			None
...
// pub struct LayerValCursor<'a, Key: Ord+Debug+'a, Val: Ord+Debug+'a, Time: Lattice+Ord+Debug+'a> {
// 	key: &'a Key,				// key for this cursor
// 	off: usize,					// current offset in *times*.
// 	vals: &'a [(Val, usize)],	// all relevant values (missing prior offset, stashed in self.off)
// 	times: &'a [(Time, isize)],	// all times (because offsets relative to whole array.)
// }

// impl<'a, 'b, Key: Ord+Debug+'a, Val: Ord+Debug+'a, Time: Lattice+Ord+Debug+'a> ValCursor<'b, Key, Val, Time> for LayerValCursor<'a, Key, Val, Time> where 'a: 'b {
// 	type TimeCursor = LayerTimeCursor<'b, Val, Time>;

// 	fn next_val(&'b mut self) -> Option<Self::TimeCursor> {
// 		if self.vals.len() > 0 {
// 			let result = LayerTimeCursor { val: &self.vals[0].0, slice: &self.times[self.off .. self.vals[0].1] };
// 			self.off = self.vals[0].1;
// 			self.vals = &self.vals[1..];
// 			Some(result)
// 		}
// 		else { None }
// 	}	
// 	fn seek_val(&'b mut self, val: &Val) -> Option<Self::TimeCursor> {
// 		let step = advance(self.vals, |x| x.0.lt(val));
// 		if step > 0 {
// 			self.off = self.vals[step-1].1;
// 			self.vals = &self.vals[step..];
// 		}
// 		if self.peek_val().map(|v| v == val) == Some(true) {
// 			self.next_val()
// 		}
// 		else {
// 			None
...
// 		if self.vals.len() > 0 { Some(&self.vals[0].0) } else { None }
// 	}
// 	fn key(&self) -> &Key { self.key }
// }

// pub struct LayerTimeCursor<'a, Val:'a, Time:'a> {
// 	val: &'a Val,
// 	slice: &'a [(Time, isize)],
// }

// impl<'a, Val: Ord+Debug+'a, Time: Lattice+Ord+Debug+'a> TimeCursor<Val, Time> for LayerTimeCursor<'a, Val, Time> {
// 	fn val(&self) -> &Val { &self.val }
// 	fn map<L: FnMut(&Time, isize)>(&self, mut logic: L) {
// 		for &(ref time, diff) in &self.slice[..] {
// 			logic(time, diff)
// 		}
// 	}
// }