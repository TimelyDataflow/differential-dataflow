...
//! pair of corgi columns, with per-tuple times held as lanes in a [`ColTimes`] (a product of
//! integers per row, no `PointStamp` materialized); diffs stay a `Vec`.
...
use crate::corgi::col_times::{ColTime, ColTimes, Lanes};
...
impl<T: Lanes, R> Default for CorgiChunk<T, R> {
    fn default() -> Self {
    ...
    R: Semigroup + Clone + 'static,
{
    ...
    fn advance(
        ...
        // Advance every complete row by the frontier in one lane-wise pass, then consolidate
        // each group: its rows share `(key, val)`, so one representative offset materializes
        // each output row's kv, and rows with equal advanced times sum. Advancing can reorder a
        // group's times, so each group is put back in lane order first (small, nearly sorted).
        ...
        for &g_end in &bounds {
            ...
            while k < order.len() {
                ...
                if !d.is_zero() {
                    tags.push(0); offs.push(i); otimes.push_ref(&ctimes, rep); odiffs.push(d);
    ...
    R: Semigroup + Clone,
{
    ...
    // they are equal; reorder the time lanes and the diffs by the same permutation.
    ...
    let (mut keep, mut ot, mut od) = (Vec::new(), ColTimes::new(), Vec::new());
    let mut run = Vec::new();
    let mut i = 0;
    while i < n {
        let mut j = i + 1;
        ...
        while k < run.len() {
            ...
            k += 1;
            while k < run.len() && times_s.cmp(run[k], rep) == Ordering::Equal {
    ...
    fn cancelled_merge_does_not_retain_input_sized_diff_storage() {
        ...
        for rows in [16, 256, 4096] {
            let make = |diffs| CorgiChunk::from_columns(
                ...
                vec![0u64; rows as usize].into_iter().collect(), diffs,
            );
            ...
            for size in [1, 3, rows.len()] {
                for shared in [false, true] {
                    let chunks: Vec<_> = rows.chunks(size).map(|rows| CorgiChunk::from_columns(
                        ...
                        rows.iter().map(|r| r.1.clone()).collect::<ColTimes<_>>(), rows.iter().map(|r| r.2).collect(),
                    )).collect();
