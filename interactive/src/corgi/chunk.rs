...
//! graded `ChunkUpdatesMerger` for free.
//!
...
use differential_dataflow::trace::chunk::{pack, Chunk, ChunkUpdates};
...
/// Build a `ChunkUpdates<CorgiChunk>` from corgi key/val COLUMNS directly (no transcode): sort +
...
pub fn columns_to_batch<T, R>(keys: CValue, vals: CValue, times: Vec<T>, diffs: Vec<R>) -> ChunkUpdates<CorgiChunk<T, R>>
where
    T: ColTime,
    R: Semigroup + Clone + 'static,
{
...
/// Grade one chunk into a `ChunkUpdates` (shared tail of `rows_to_batch`/`columns_to_batch`).
fn settle_one<T, R>(chunk: CorgiChunk<T, R>) -> ChunkUpdates<CorgiChunk<T, R>>
where
    T: ColTime,
    R: Semigroup + Clone + 'static,
{
...
mod test {
    use super::*;
    use differential_dataflow::trace::chunk::{ChunkUpdatesMerger, is_graded};
    ...
    fn read_batch(b: &ChunkUpdates<CorgiChunk<u64, i64>>) -> BTreeMap<((u64, u64), u64), i64> {
    ...
    fn batch_merger_resumable_matches_reference() {
        ...
        for _ in 0..200 {
            ...
            let mut merger = ChunkUpdatesMerger::new(&s1, &s2, frontier.borrow());
            loop {
