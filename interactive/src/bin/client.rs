...
fn main() {
    ...
    for node in 0 .. 1000 {
        ...
        Command::<Value>::UpdateInput("Edges".to_string(), vec![(edge, Duration::from_secs(0), 1)]).serialize_into(&mut socket);
    }

    // Create initially empty set of edges.
    Command::<Value>::CreateInput("Nodes".to_string(), Vec::new()).serialize_into(&mut socket);
    ...
    Command::<Value>::UpdateInput("Nodes".to_string(), vec![(vec![Value::Usize(0)], Duration::from_secs(1), 1)]).serialize_into(&mut socket);
    ...
    Command::<Value>::AdvanceTime(Duration::from_secs(3)).serialize_into(&mut socket);
