...
impl Backend for VecBackend {
    ...
    // The vec container *is* the row container, so both conversions are the identity.
    fn to_rows<'s>(c: Collection<'s, Time, Self::Container>) -> Col<'s> { c }
    fn from_rows<'s>(c: Col<'s>) -> Collection<'s, Time, Self::Container> { c }
    ...
    fn delta_stage<'s>(
        ...
        half_join(requests, index, no_compaction, strict, move |_k, prefix, val| stage_extend(prefix, bind, val))
    }
