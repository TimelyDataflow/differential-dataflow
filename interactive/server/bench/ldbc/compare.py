...
def main():
    ...
    suite = before['format_version'] == 'snb-suite-1'
    for report in (before, after):
        if report['status'] != 'passed' or report['format_version'] != ('snb-suite-1' if suite else 1):
            parser.error('both reports must be successful runs of the same supported format')
    ...
    sources = ('client.py', 'run.py', 'suite.py', 'snb/data.py', 'snb/parameters.py', 'snb/witness.py') if suite else ('client.py', 'workload.py', 'run.py')
