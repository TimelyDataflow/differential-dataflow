...
for g in `ls results/$commit/i-tpchlike-mixing-jemalloc | cut -d '_' -f 4-6 | sort | uniq`; do
  ...
  gnuplot -p -e "$plotscript" > plots/$commit/i-tpchlike-mixing-jemalloc/ccdf-install-$g.pdf
done
...
for g in `ls results/$commit/i-tpchlike-mixing-jemalloc | cut -d '_' -f 4-6 | sort | uniq`; do
  ...
  gnuplot -p -e "$plotscript" > plots/$commit/i-tpchlike-mixing-jemalloc/ccdf-uninstall-$g.pdf
done
...
for g in `ls results/$commit/i-tpchlike-mixing-jemalloc | cut -d '_' -f 4-6 | sort | uniq`; do
  ...
  gnuplot -p -e "$plotscript" > plots/$commit/i-tpchlike-mixing-jemalloc/ccdf-work-$g.pdf
done
