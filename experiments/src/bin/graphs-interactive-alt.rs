    ...
    timely::execute_from_args(std::env::args().skip(3), move |worker| {
        ...
        let (mut q1, mut q2, mut q3, mut q4, mut state, mut graph) = worker.dataflow(|scope| {
            ...
            if shared {
                ...
                three_hop(graph_indexed.clone(), graph_indexed.clone(), q4.clone())
                    .probe_with(&mut probe);
                ...
                // let state_indexed = state.arrange_by_key();
                let graph = graph.clone().map(|(src, dst)| (dst, src))
                                 .concat(graph);

                // Q1: Point lookups on `state`:
                q1  .arrange_by_self()
                    .join_core(state.clone().arrange_by_key(), |&query, &(), &state| Some((query, state)))
                    .probe_with(&mut probe);

                // Q2: One-hop lookups on `state`:
                q2  .arrange_by_self()
                    .join_core(graph.clone().arrange_by_key(), |&query, &(), &friend| Some((friend, query)))
                    .join_core(state.clone().arrange_by_key(), |_friend, &query, &state| Some((query, state)))
                    .probe_with(&mut probe);

                // Q3: Two-hop lookups on `state`:
                q3  .arrange_by_self()
                    .join_core(graph.clone().arrange_by_key(), |&query, &(), &friend| Some((friend, query)))
                    .join_core(graph.clone().arrange_by_key(), |_friend, &query, &friend2| Some((friend2, query)))
                    .join_core(state.arrange_by_key(), |_friend2, &query, &state| Some((query, state)))
                    .probe_with(&mut probe);

                // Q4: Shortest path queries:
                three_hop(graph.clone().arrange_by_key(), graph.clone().arrange_by_key(), q4.clone())
                    .probe_with(&mut probe);
...
type Arrange<G, K, V, R> = Arranged<G, TraceIntra<ValSpine<K, V, <G as ScopeParent>::Timestamp, R>>>;
...
where G::Timestamp: Lattice+Ord {
    ...
    goals.scope().iterative::<Iter,_,_>(|inner| {
        ...
        let (forward, forward_collection) = Variable::new_from(goals.clone().map(|(x,_)| (x,(x,0))).enter(inner), Product::new(Default::default(), 1));
        let (reverse, reverse_collection) = Variable::new_from(goals.clone().map(|(_,y)| (y,(y,0))).enter(inner), Product::new(Default::default(), 1));
