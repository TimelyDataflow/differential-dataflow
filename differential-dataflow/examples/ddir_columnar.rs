    ...
    timely::execute_from_args(std::env::args().skip(4), move |worker| {
        ...
        if std::env::var("DDIR_REACHABILITY").is_ok() {
            ...
            if let Some(mut registry) = worker.log_register() {
                registry.insert::<TrackerEventBuilder<u64>,_>(&outer_name, |_time, data| {
                    for (_ts, event) in data.iter().flat_map(|v| v.iter()) {
                        match &event {
                            TrackerEvent::SourceUpdate(s) => {
                                for &(node, port, ref t, diff) in &s.updates {
                                    if diff != 0 { eprintln!("  [reach/u64] SRC tracker={} node={} port={} time={:?} diff={}", s.tracker_id, node, port, t, diff); }
                                }
                            },
                            TrackerEvent::TargetUpdate(t_upd) => {
                                for &(node, port, ref t, diff) in &t_upd.updates {
                                    if diff != 0 { eprintln!("  [reach/u64] TGT tracker={} node={} port={} time={:?} diff={}", t_upd.tracker_id, node, port, t, diff); }
                                }
                            },
                        }
                    }
                });
                registry.insert::<TrackerEventBuilder<DdirTime>,_>(&inner_name, |_time, data| {
                    for (_ts, event) in data.iter().flat_map(|v| v.iter()) {
                        match &event {
                            TrackerEvent::SourceUpdate(s) => {
                                for &(node, port, ref t, diff) in &s.updates {
                                    if diff != 0 { eprintln!("  [reach/ddir] SRC tracker={} node={} port={} time={:?} diff={}", s.tracker_id, node, port, t, diff); }
                                }
                            },
                            TrackerEvent::TargetUpdate(t_upd) => {
                                for &(node, port, ref t, diff) in &t_upd.updates {
                                    if diff != 0 { eprintln!("  [reach/ddir] TGT tracker={} node={} port={} time={:?} diff={}", t_upd.tracker_id, node, port, t, diff); }
                                }
                            },
                        }
                    }
