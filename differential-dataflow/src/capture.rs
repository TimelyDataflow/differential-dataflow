...
pub mod sink {
        ...
        R: ExchangeData + Hash + Serialize + for<'a> Deserialize<'a>,
    {
        ...
        let mut builder = OperatorBuilder::new("UpdatesWriter".to_owned(), scope.clone());
        let reactivator = scope.activator_for(builder.operator_info().address);
        ...
        let mut builder = OperatorBuilder::new("ProgressWriter".to_owned(), scope.clone());
        let reactivator = scope.activator_for(builder.operator_info().address);
        let mut input = builder.new_input(updates, Exchange::new(move |_| sink_hash));
        let should_write = scope.index() == (sink_hash as usize) % scope.peers();
