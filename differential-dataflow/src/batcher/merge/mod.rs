...
pub mod vec {
    ...
    use crate::batcher::merge::Merger;
...
mod test {
    ...
    use crate::batcher::Batcher;
    use super::{MergeBatcher, Sealer};
    use super::vec::VecMerger;
    use crate::batcher::merge::chunker::ContainerChunker;

    type In = Vec<((u64, ()), u64, i64)>;
    type Bt = MergeBatcher<ContainerChunker<In>, VecMerger<(u64, ()), u64, i64>, TestSealer>;

    struct TestSealer;

    impl Sealer<In> for TestSealer {
        type Output = Vec<In>;

        fn seal(chain: &mut Vec<In>) -> Option<Self::Output> {
            (!chain.is_empty()).then(|| std::mem::take(chain))
        }
    }
