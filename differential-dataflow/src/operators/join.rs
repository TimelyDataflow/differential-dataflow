...
where
    ...
    L: FnMut(columnar::Ref<'_,T1::Key>,columnar::Ref<'_,T1::Val>,columnar::Ref<'_,T2::Val>,&G::Timestamp,&T1::Diff,&T2::Diff,&mut JoinSession<T1::Time, CB, Capability<T1::Time>>)+'static,
    CB: ContainerBuilder,
{
...
where
    T: Timestamp+Lattice+Ord+crate::Data,
    C1: Cursor<Time=T>,
    ...
    T: Timestamp+Lattice+Ord+crate::Data,
{
    ...
    where
        L: for<'a> FnMut(columnar::Ref<'a, C1::Key>, columnar::Ref<'a, C1::Val>, columnar::Ref<'a, C2::Val>, &T, &C1::Diff, &C2::Diff, &mut JoinSession<T, CB, Capability<T>>),
    {
        ...
        while let (Some(batch_key), Some(trace_key), true) = (batch.get_key(batch_storage), trace.get_key(trace_storage), effort < *fuel) {
            ...
            match trace_key.cmp(&batch_key) {
                ...
                Ordering::Equal => {
                    ...
                    thinker.history2.edits.load(batch, batch_storage, |time| <C2::Time as columnar::Columnar>::into_owned(time));
