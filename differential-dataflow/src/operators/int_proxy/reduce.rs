...
pub use tactic::ProxyReduceTactic;
mod tactic {
    ...
    use crate::operators::int_proxy::debug_assert_sorted_bridge;
    ...
    ///
    /// Holds a backend, and list of pending (hash, time) pairs for future invocations.
    ...
    impl<T, Bk> ProxyReduceTactic<T, Bk> {
        ...
        pub fn new(backend: Bk) -> Self {
            ProxyReduceTactic { backend, pending_keys: Vec::new(), pending_time: Vec::new() }
        }
    }
        ...
        Bk: ProxyReduceBackend<B1, B2>,
    {
        fn retire(
            ...
            let mut scratch = Scratch::<B1::Time, Bk::RIn, Bk::ROut>::new();
            ...
            while from.is_some() {
                ...
                debug_assert_sorted_bridge(&window.history, "next_window.history");
                debug_assert_sorted_bridge(&window.novel, "next_window.novel");
                debug_assert_sorted_bridge(&window.output, "next_window.output");
                ...
                for (held_index, deltas) in scratch.tile_deltas.iter_mut().enumerate() {
                    if deltas.is_empty() {
                        continue;
                    }
    ...
    /// The working state one retire builds and tears down: each piece is cleared or refilled at its
    /// natural boundary — the window, the wave, or the tick — and the whole evaporates when the
    /// retire returns. Fresh per-key/per-wave `Vec`s were once the dominant cost of the tactic, so
    /// the reuse that matters is within a retire, where the window loop and the wave loop run many
    /// times over.
    struct Scratch<T, RIn, ROut> {
        ...
        /// Per-key sweeps, reused across the retire's windows.
        slots: Vec<KeySweep<T, RIn, ROut>>,
        /// The slots (indices) still suspended at a time, i.e. participating in the next wave.
        live: Vec<usize>,
        /// Output updates staged per held time, drained into `emit` per window. One entry per held
        /// time, allocated once and cleared per window.
