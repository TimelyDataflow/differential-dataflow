    ...
    TOuter: Timestamp,
{
    ...
    pub fn leave_dynamic(&self, level: usize) -> Self {
        ...
        let connection = [(0, Antichain::from_elem(Product { outer: Default::default(), inner: PointStampSummary { retain: Some(level - 1), actions: Vec::new() } }))].into();
        let mut input = builder.new_input_connection(&self.inner, Pipeline, connection);
