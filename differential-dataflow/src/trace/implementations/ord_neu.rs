...
pub mod val_batch {
    ...
    where
        OrdValBatch<U>: Batch<Time=U::Time>,
    {
        fn new(batch1: &OrdValBatch<U>, batch2: &OrdValBatch<U>, compaction_frontier: AntichainRef<U::Time>) -> Self {
            ...
            OrdValMerger {
                ...
                result: OrdValStorage {
                    keys: Vec::with_capacity(batch1.keys.len() + batch2.keys.len()),
    ...
    impl<U: Update> OrdValMerger<U> {
        ...
        fn stash_updates_for_val(&mut self, source: &OrdValStorage<U>, index: usize) {
            ...
            for i in lower .. upper {
                ...
                self.staging.push(new_time, diff.clone());
            }
        }
    }
    ...
    impl<U: Update> Cursor for OrdValCursor<U> {
        ...
        fn seek_key(&mut self, storage: &OrdValBatch<U>, key: &Self::Key) {
...
pub mod key_batch {
    ...
    where
        OrdKeyBatch<U>: Batch<Time=U::Time>,
    {
