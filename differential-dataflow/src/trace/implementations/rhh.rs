...
pub type VecSpine<K, V, T, R> = Spine<Rc<RhhValBatch<((K,V),T,R)>>>;
...
pub type VecBuilder<K,V,T,R> = RcBuilder<RhhValBuilder<((K,V),T,R)>>;
...
mod val_batch {
    ...
    pub struct RhhValStorage<U: Update>
    where
        U::Key: Default + HashOrdered,
    {
        ...
        pub times: Coltainer<U::Time>,
        /// Concatenated ordered lists of update diffs, bracketed by offsets in `vals_offs`.
        pub diffs: Coltainer<U::Diff>,
    }

    impl<U: Update> RhhValStorage<U>
    where
        U::Key: Default + HashOrdered,
        for<'a> columnar::Ref<'a, U::Key>: HashOrdered,
    {
        ...
        fn insert_key(&mut self, key: columnar::Ref<'_, U::Key>, offset: Option<usize>) {
            ...
            while self.keys.len() < desired {
                ...
                self.keys.push_own(&<U::Key as Default>::default());
        ...
        fn insert_key_own(&mut self, key: &U::Key, offset: Option<usize>) {
            let mut key_con = Coltainer::<U::Key>::with_capacity(1);
        ...
        fn advance_key(&self, index: usize, key: columnar::Ref<'_, U::Key>) -> bool {
            // Ideally this short-circuits, as `self.keys[index]` is bogus data.
            !self.live_key(index) || self.keys.index(index).lt(&Coltainer::<U::Key>::reborrow(key))
        }
    ...
    ///
    /// The `U` parameter captures the update type (key, val, time, diff).
    pub struct RhhValBatch<U: Update>
    where
        U::Key: Default + HashOrdered,
    {
        /// The updates themselves.
        pub storage: RhhValStorage<U>,
    ...
    impl<U: Update> BatchReader for RhhValBatch<U>
    where
        U::Key: Default + HashOrdered,
        for<'a> columnar::Ref<'a, U::Key>: HashOrdered,
    {
    ...
    impl<U: Update> Batch for RhhValBatch<U>
    where
        U::Key: Default + HashOrdered,
        for<'a> columnar::Ref<'a, U::Key>: HashOrdered,
    {
        type Merger = RhhValMerger<U>;
        ...
        fn empty(lower: Antichain<Self::Time>, upper: Antichain<Self::Time>) -> Self {
            ...
            Self {
                storage: RhhValStorage {
                    keys: Coltainer::<U::Key>::with_capacity(0),
                    keys_offs: OffsetList::with_capacity(0),
                    vals: Coltainer::<U::Val>::with_capacity(0),
                    vals_offs: OffsetList::with_capacity(0),
                    times: Coltainer::<U::Time>::with_capacity(0),
                    diffs: Coltainer::<U::Diff>::with_capacity(0),
                    key_count: 0,
                    key_capacity: 0,
                    divisor: 0,
                },
    ...
    pub struct RhhValMerger<U: Update>
    where
        U::Key: Default + HashOrdered,
    {
        ...
        update_stash: Vec<(U::Time, U::Diff)>,
        /// Counts the number of singleton-optimized entries, that we may correctly count the updates.
        singletons: usize,
    }

    impl<U: Update> Merger<RhhValBatch<U>> for RhhValMerger<U>
    where
        U::Key: Default + HashOrdered,
        for<'a> columnar::Ref<'a, U::Key>: HashOrdered,
    {
        fn new(batch1: &RhhValBatch<U>, batch2: &RhhValBatch<U>, compaction_frontier: AntichainRef<U::Time>) -> Self {
            ...
            let mut storage = RhhValStorage {
                keys: Coltainer::<U::Key>::merge_capacity(&batch1.keys, &batch2.keys),
                keys_offs: OffsetList::with_capacity(batch1.keys_offs.len() + batch2.keys_offs.len()),
                vals: Coltainer::<U::Val>::merge_capacity(&batch1.vals, &batch2.vals),
                vals_offs: OffsetList::with_capacity(batch1.vals_offs.len() + batch2.vals_offs.len()),
                times: Coltainer::<U::Time>::merge_capacity(&batch1.times, &batch2.times),
                diffs: Coltainer::<U::Diff>::merge_capacity(&batch1.diffs, &batch2.diffs),
                key_count: 0,
                key_capacity: rhh_cap,
                divisor: RhhValStorage::<U>::divisor_for_capacity(rhh_cap),
            };

            // Mark explicit types because type inference fails to resolve it.
            let keys_offs: &mut OffsetList = &mut storage.keys_offs;
            keys_offs.push_ref(0);
            let vals_offs: &mut OffsetList = &mut storage.vals_offs;
        ...
        fn work(&mut self, source1: &RhhValBatch<U>, source2: &RhhValBatch<U>, fuel: &mut isize) {
        ...
        for<'a> columnar::Ref<'a, U::Key>: HashOrdered,
    {
        ...
        fn copy_key(&mut self, source: &RhhValStorage<U>, cursor: usize) {
        ...
        fn merge_key(&mut self, source1: &RhhValStorage<U>, source2: &RhhValStorage<U>) {
        ...
        fn merge_vals(
            &mut self,
            (source1, mut lower1, upper1): (&RhhValStorage<U>, usize, usize),
            (source2, mut lower2, upper2): (&RhhValStorage<U>, usize, usize),
        ) -> Option<usize> {
            // Capture the initial number of values to determine if the merge was ultimately non-empty.
            let init_vals = self.result.vals.len();
        ...
        fn stash_updates_for_val(&mut self, source: &RhhValStorage<U>, index: usize) {
            ...
            for i in lower .. upper {
                ...
                let mut new_time = <U::Time as columnar::Columnar>::into_owned(time);
                use crate::lattice::Lattice;
                new_time.advance_by(self.description.since().borrow());
                self.update_stash.push((new_time, <U::Diff as columnar::Columnar>::into_owned(diff)));
            }
        }
        ...
        fn consolidate_updates(&mut self) -> Option<usize> {
            ...
            if !self.update_stash.is_empty() {
                ...
                let last_eq = self.update_stash.last().zip(time_diff).map(|((t1, d1), (t2, d2))| {
                    ...
                    *t1 == <U::Time as columnar::Columnar>::into_owned(t2) && *d1 == <U::Diff as columnar::Columnar>::into_owned(d2)
                });
    ...
    pub struct RhhValCursor<U: Update>
    where
        U::Key: Default + HashOrdered,
    {
    ...
    impl<U: Update> Cursor for RhhValCursor<U>
    where
        U::Key: Default + HashOrdered,
        for<'a> columnar::Ref<'a, U::Key>: HashOrdered,
    {
        ...
        fn get_key<'a>(&self, storage: &'a RhhValBatch<U>) -> Option<columnar::Ref<'a, Self::Key>> { storage.storage.keys.get(self.key_cursor) }
        fn get_val<'a>(&self, storage: &'a RhhValBatch<U>) -> Option<columnar::Ref<'a, Self::Val>> { if self.val_valid(storage) { Some(self.val(storage)) } else { None } }
        fn key<'a>(&self, storage: &'a RhhValBatch<U>) -> columnar::Ref<'a, Self::Key> { storage.storage.keys.index(self.key_cursor) }
        fn val<'a>(&self, storage: &'a RhhValBatch<U>) -> columnar::Ref<'a, Self::Val> { storage.storage.vals.index(self.val_cursor) }
        fn map_times<L2: FnMut(columnar::Ref<'_, Self::Time>, columnar::Ref<'_, Self::Diff>)>(&mut self, storage: &RhhValBatch<U>, mut logic: L2) {
        ...
        fn key_valid(&self, storage: &RhhValBatch<U>) -> bool { self.key_cursor < storage.storage.keys.len() }
        fn val_valid(&self, storage: &RhhValBatch<U>) -> bool { self.val_cursor < storage.storage.values_for_key(self.key_cursor).1 }
        fn step_key(&mut self, storage: &RhhValBatch<U>){
        ...
        fn seek_key(&mut self, storage: &RhhValBatch<U>, key: columnar::Ref<'_, Self::Key>) {
        ...
        fn step_val(&mut self, storage: &RhhValBatch<U>) {
            self.val_cursor += 1;
            if !self.val_valid(storage) {
                self.val_cursor = storage.storage.values_for_key(self.key_cursor).1;
            }
        }
        fn seek_val(&mut self, storage: &RhhValBatch<U>, val: columnar::Ref<'_, Self::Val>) {
            self.val_cursor += storage.storage.vals.advance(self.val_cursor, storage.storage.values_for_key(self.key_cursor).1, |x| Coltainer::<U::Val>::reborrow(x).lt(&Coltainer::<U::Val>::reborrow(val)));
        }
        fn rewind_keys(&mut self, storage: &RhhValBatch<U>) {
            self.key_cursor = 0;
            storage.storage.advance_to_live_key(&mut self.key_cursor);
        ...
        fn rewind_vals(&mut self, storage: &RhhValBatch<U>) {
            self.val_cursor = storage.storage.values_for_key(self.key_cursor).0;
        }
    }

    /// A builder for creating layers from unsorted update tuples.
    pub struct RhhValBuilder<U: Update>
    where
        U::Key: Default + HashOrdered,
    {
        result: RhhValStorage<U>,
        singleton: Option<(U::Time, U::Diff)>,
        ...
        U::Key: Default + HashOrdered,
    {
        ...
        fn push_update(&mut self, time: U::Time, diff: U::Diff) {
            ...
            if self.result.times.last().map(|t| <U::Time as columnar::Columnar>::into_owned(t) == time).unwrap_or(false) && self.result.diffs.last().map(|d| <U::Diff as columnar::Columnar>::into_owned(d) == diff).unwrap_or(false) {
    ...
    impl<U: Update> Builder for RhhValBuilder<U>
    where
        U::Key: Default + HashOrdered,
        for<'a> columnar::Ref<'a, U::Key>: HashOrdered,
    {
        ...
        type Time = U::Time;
        type Output = RhhValBatch<U>;

        fn with_capacity(keys: usize, vals: usize, upds: usize) -> Self {

            // Double the capacity for RHH; probably excessive.
            let rhh_capacity = 2 * keys;
            let divisor = RhhValStorage::<U>::divisor_for_capacity(rhh_capacity);
            ...
            Self {
                result: RhhValStorage {
                    keys: Coltainer::<U::Key>::with_capacity(keys),
                    keys_offs: OffsetList::with_capacity(keys + 1),
                    vals: Coltainer::<U::Val>::with_capacity(vals),
                    vals_offs: OffsetList::with_capacity(vals + 1),
                    times: Coltainer::<U::Time>::with_capacity(upds),
                    diffs: Coltainer::<U::Diff>::with_capacity(upds),
                    key_count: 0,
        ...
        fn push(&mut self, chunk: &mut Self::Input) {
            for ((key, val), time, diff) in chunk.drain(..) {
                ...
                if self.result.keys.last().map(|k| Coltainer::<U::Key>::into_owned(k) == key).unwrap_or(false) {
                    // Perhaps this is a continuation of an already received value.
                    if self.result.vals.last().map(|v| Coltainer::<U::Val>::into_owned(v) == val).unwrap_or(false) {
        ...
        #[inline(never)]
        fn done(mut self, description: Description<Self::Time>) -> RhhValBatch<U> {
        ...
        fn seal(chain: &mut Vec<Self::Input>, description: Description<Self::Time>) -> Self::Output {
            let mut keys = 0; let mut vals = 0; let mut upds = 0;
            for link in chain.iter() {
                let mut prev_keyval = None;
                for ((key, val), _, _) in link.iter() {
                    if let Some((pk, pv)) = prev_keyval {
                        if pk != key { keys += 1; vals += 1; }
                        else if pv != val { vals += 1; }
                    } else { keys += 1; vals += 1; }
                    upds += 1;
